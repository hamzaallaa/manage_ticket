    ...
    >
      <div v-show="dropdownOpen" class="origin-top-right z-10 absolute top-full min-w-44 bg-white border border-slate-200 py-1.5 rounded shadow-lg overflow-hidden mt-1" :class="align === 'right' ? 'right-0' : 'left-0'">
        <div class="pt-0.5 pb-2 px-3 mb-1 border-b border-slate-200">
          <div class="font-medium text-slate-800 capitalize">{{user?.name}}</div>
          <div class="text-xs text-slate-500 italic uppercase">{{user?.role}}</div>
        </div>
        <ul
          ref="dropdown"
        ...
        >
          <li>
            <router-link class="font-medium text-sm text-lime-500 hover:text-lime-600 flex items-center py-1 px-3" to="/signin" @click="SignOut">Sign Out</router-link>
          </li>
        </ul>
      </div> 
    </transition>
